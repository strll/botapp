<template>
	<view>
		<view>
			<u--input placeholder="请输入用户名(登陆时通过用户名登陆)" border="surround" v-model="username" @change="change">
		...
		methods: {
			register() {
				...
				if (this.username == '') {
					...
					uni.request({
						url: "http://150.158.77.254:9991/user/register?username=" + this.username + "&password=" +
			...
			getcode() {
				...
				if (!emailRegex.test(this.email)) {
					...
					uni.request({
						url: "http://150.158.77.254:9991/user/verification_code?email=" + this.email,
						success: ref => {
							uni.showToast({
								icon: 'success',
								title: '请查看你邮箱的验证码',
